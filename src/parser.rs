...
impl Parser {
	...
	fn parse_program(&mut self) -> Result<ast::Program, Box<dyn std::error::Error>> {
		return Err(Box::new(ParserError(String::new())))
	}
}
...
mod tests {
	...
	fn test_let_statements() {
		...
		if program.statements.len() != 3 {
			panic!("programStatements does not contain 3 statements. got={}", program.statements.len())
		}
		let tests = vec![
			TestIdent("x"), TestIdent("y"), TestIdent("foobar")
		];
	...
	fn test_let_statement(actual: &Statement, expected: &TestIdent) -> bool {
		return match actual {
			Statement::Let(stmt) if stmt.token_literal().as_ref() != "let" => {
				eprintln!("s.TokenLiteral not 'let'. got={}", stmt);
				false
			},
			Statement::Let(stmt) if &stmt.name.value != expected.0 => {
				eprintln!("letStmt.Name.Value not '{}'. got={}", expected.0, &stmt.name.value);
				false
			},
			Statement::Let(stmt) if &stmt.name.token_literal() != expected.0 => {
				eprintln!("letStmt.Name.TokenLiteral not '{}'. got={}", expected.0, &stmt.name.token_literal());
				false
			}
